...
TEST(runUnitTests, TestGetAndSetTimestampsIntervals) {
  ...
  EXPECT_EQ(differentialTimestamps.getTimestampsIntervals(), timestamps_intervals_ms);
}
...
TEST(runUnitTests, TestCalculateFirstTimestampInBlock) {
  ...
  std::vector<uint32_t> block_intervals_ms1 = {0, 113, 23, 34};  // first block_intervall[0] is always null - one loop less is needed
  uint32_t blockIdx = 2;
  DeserializedDifferentialTimestamps differentialTimestamps1;
  differentialTimestamps1.setFirstTimestamp(first_timestamp_ms1);
  differentialTimestamps1.setBlockIntervals(block_intervals_ms1);
  uint32_t firstTimestampInBlock = differentialTimestamps1.calculateFirstTimestampInBlock(blockIdx);
...
TEST(runUnitTests, TestCalculateLastTimestampInBlock) {
  ...
  uint32_t lastTimestampInBlock =
      differentialTimestamps1.calculateLastTimestampInBlock(blockIdx, firstTimestampInBlock, deserializedDifferentialBlock);
...
TEST(runUnitTests, TestCalculateLastTimestampInBlockException) {
  try {
    ...
    uint32_t blockIdx = 7;
    std::vector<uint32_t> timestamps_intervals_ms = {134, 31, 124};
    std::vector<uint32_t> block_intervals_ms1 = {0, 113, 23, 34};  // first block_intervall[0] is always null - one loop
                                                                   // less is needed
    DeserializedDifferentialTimestamps differentialTimestamps1;
    differentialTimestamps1.setFirstTimestamp(first_timestamp_ms1);
    differentialTimestamps1.setBlockIntervals(block_intervals_ms1);
    differentialTimestamps1.setTimestampsIntervals(timestamps_intervals_ms);
    uint32_t firstTimestampInBlock = differentialTimestamps1.calculateFirstTimestampInBlock(2);  // firstTimestampInBlock = 146
    std::vector<int32_t> diffValues = {30, 32, 54};
    DeserializedDifferentialBlock deserializedDifferentialBlock;
    deserializedDifferentialBlock.setDiffValues(diffValues);
    uint32_t lastTimestampInBlock =
    ...
    EXPECT_EQ(e.what(), std::string("blockIdx is higher than number of block_intervals_ms"));
  } catch (...) {
    FAIL() << "Expected std::invalid_argument";
  }
}